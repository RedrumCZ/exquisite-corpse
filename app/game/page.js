      ...
      <Screen>
        ...
        {isHost ? (
          <>
            ...
            {roomState.players.length < 2 && (
              <p style={st.hint}>{t.needTwoPlayers}</p>
            )}
          </>
        ) : (
...
function InputPhaseScreen({ roomState, lang, alreadyAnswered, onSubmit }) {
  ...
  const phaseIndex = roomState.currentPhaseIndex;

  // Reset local state when phase index changes (new question)
  // This is the COMPONENT-level reset — the hook-level reset handles alreadyAnswered
  useEffect(() => {
    setAnswer("");
    setSubmitting(false);
  ...
  const waitingFor    = Math.max(0, activeCount - answeredCount);

  // Phase hint — index-matched example text
  const phaseHint = t.phaseHints?.[phaseIndex] ?? null;
    ...
    <Screen>
      ...
      <div style={st.progressDots}>
        {Array.from({ length: roomState.totalPhases }).map((_, i) => (
          <div key={i} style={{
            ...st.dot,
            background: i < phaseIndex ? "#2ecc71"
              : i === phaseIndex ? "#e74c3c" : "#333",
            transform: i === phaseIndex ? "scale(1.3)" : "scale(1)",
          }} />
        ))}
      </div>

      <p style={st.phaseCounter}>
        {t.question(phaseIndex + 1, roomState.totalPhases)}
      </p>

      <div style={st.phaseLabel}>{roomState.phaseLabel}</div>

      {phaseHint && <p style={st.phaseHint}>{phaseHint}</p>}
      ...
      {!submitted ? (
        <>
          ...
          />
          <p style={{ ...st.hint, alignSelf: "flex-end", fontSize: "0.72rem" }}>
            {answer.length}/280
          </p>
          <button
...
function FinalResultsScreen({ finalResults, playerId, isHost, lang, onRestart, onHome }) {
  ...
  return (
    <Screen>
      ...
      {isHost ? (
        <button style={st.btn} onClick={onRestart}>{t.playAgain}</button>
      ) : (
        <p style={st.hint}>⏳ {t.waitingNextRound}</p>
      )}

      <button style={st.btnSecondary} onClick={onHome}>{t.backHome}</button>
    </Screen>
  );
}
...
function Screen({ children }) {
  return <main style={st.main}>{children}</main>;
}
...
const st = {
  main:         { minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"flex-start", padding:"2rem 1rem 4rem", gap:"1.25rem" },
  ...
  hiddenHint:   { color:"#666", fontSize:"0.74rem", margin:0, fontStyle:"italic" },
  phaseHint:    { color:"#555", fontSize:"0.78rem", margin:0, fontStyle:"italic", textAlign:"center", maxWidth:"480px" },
  ...
  progressDots: { display:"flex", gap:"0.4rem", alignItems:"center" },
  dot:          { width:"10px", height:"10px", borderRadius:"50%", transition:"background 0.3s, transform 0.2s" },
